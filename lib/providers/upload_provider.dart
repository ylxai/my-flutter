...
class UploadNotifier extends StateNotifier<UploadState> {
  ...
  Future<void> startUpload(UploadConfig config) async {
    ...
    if (!connected) {
      ...
      state = state.copyWith(
        status: UploadStatus.error,
        errorMessage:
            'Cannot connect to R2. Check credentials.',
      );
    ...
    if (config.uploadOriginalToDrive) {
      ...
      if (config.googleDriveCredentialsPath == null) {
        _addLog('Drive credentials not set in Settings',
        ...
        try {
          ...
          if (_driveService.isAuthenticated) {
            _addLog('Google Drive authenticated ✓',
                level: LogLevel.success);
          }
        } catch (e) {
          _addLog('Drive auth failed: $e',
              level: LogLevel.error);
          state = state.copyWith(
            status: UploadStatus.error,
    ...
    try {
      await for (final progress
          in _orchestrator!.execute(config)) {
    ...
    final msg = switch (progress.phase) {
      ...
      UploadPhase.uploadingToDrive =>
        'Uploading to Drive ${progress.currentFile}/'
            '${progress.totalFiles}: '
            '${progress.currentFileName}',
      UploadPhase.generatingManifest =>
        'Generating manifest...',
      _ => null,
    };
    if (msg != null) {
  ...
  void _saveToHistory(
    ...
    _historyNotifier.addRecord(PublishRecord(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
      eventName: config.eventName,
      publishedAt: DateTime.now(),
      photoCount: progress?.totalFiles ?? 0,
      successCount: progress?.currentFile ?? 0,
      failedCount: 0,
...
final driveServiceProvider =
    Provider<GoogleDriveUploadService>((ref) {
...
final uploadProvider =
  ...
  (ref) {
    return UploadNotifier(
      ...
      historyNotifier:
          ref.watch(publishHistoryProvider.notifier),
    );
  },
);
